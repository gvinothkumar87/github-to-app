...
const CreditNoteForm = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    reason: '',
    note_date: new Date(),
    mill: '', // Initialize empty, will set when locations load
  });
  ...
  // Set default mill when locations are loaded
  useEffect(() => {
  ...
  useEffect(() => {
    const fetchPreviewNoteNo = async () => {
      const noteNo = await generateCreditNoteNo(formData.mill, formData.note_date);
      if (noteNo) {
  ...
  const generateCreditNoteNo = async (mill: string, date: Date = new Date()) => {
    try {
      // Fetch settings
      const { data: settings } = await supabase
        .from('company_settings')
        .select('start_credit_note_no, credit_note_prefix, credit_note_digits, credit_note_financial_year_in_serial')
        .eq('location_code', mill)
        .eq('is_active', true)
        .single();

      const startNo = settings?.start_credit_note_no || 1;
      const basePrefix = settings?.credit_note_prefix || '';
      const digits = settings?.credit_note_digits || 3;
      const useFY = settings?.credit_note_financial_year_in_serial ?? false;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let vQuery = supabase.from('credit_notes').select('note_no, note_date');
      if (prefix) {
      ...
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="preview_note_no">Credit Note No. (Preview)</Label>
              <Input
                id="preview_note_no"
                value={previewNoteNo}
                readOnly
