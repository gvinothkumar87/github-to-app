...
import type { Customer, DebitNote } from "@/types";

interface EditDebitNoteFormProps {
  debitNote: DebitNote;
  customer: Customer;
  onSuccess: () => void;
  onCancel: () => void;
...
export const EditDebitNoteForm = ({ debitNote, customer: initialCustomer, onSuccess, onCancel }: EditDebitNoteFormProps) => {
  const [customers, setCustomers] = useState<Customer[]>([]);
  const [formData, setFormData] = useState({
    customer_id: debitNote.customer_id,
    reference_bill_no: debitNote.reference_bill_no || '',
    amount: debitNote.amount.toString(),
    gst_percentage: (debitNote.gst_percentage || 18).toString(),
    reason: debitNote.reason,
    note_date: parseISO(debitNote.note_date),
  });
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      <CardHeader>
        <CardTitle>Edit Debit Note - {debitNote.note_no}</CardTitle>
      </CardHeader>
      <CardContent>
