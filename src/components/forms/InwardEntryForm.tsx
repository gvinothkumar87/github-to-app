...
export const InwardEntryForm: React.FC<InwardEntryFormProps> = ({ onSuccess, onCancel }) => {
      ...
      reader.onloadend = async () => {
        try {
          ...
          if (error) {
            ...
            // specific error handling for FunctionsHttpError
            if (error instanceof Error && error.name === 'FunctionsHttpError') {
              ...
              if (context && context.json) {
                context.json().then((errBody: any) => {
                  ...
                  toast({
                    variant: 'destructive',
                    title: language === 'english' ? 'Upload Failed' : 'பதிவேற்றம் தோல்வியடைந்தது',
                    description: errBody.error || (language === 'english' ? 'Failed to upload photo' : 'புகைப்படத்தை பதிவேற்ற முடியவில்லை'),
                  });
                }).catch(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase
        ...
        .insert([{
          ...formData,
          full_weight: parseFloat(formData.full_weight),
      ...
      toast({
        ...
        description: language === 'english' ? 'Inward entry created successfully' : 'உள்வரும் பதிவு வெற்றிகரமாக உருவாக்கப்பட்டது',
      });
  ...
  return (
    <Card className="w-full max-w-2xl mx-auto shadow-card">
      <CardHeader>
        <CardTitle>
          {language === 'english' ? 'New Inward Entry' : 'புதிய உள்வரும் பதிவு'}
        </CardTitle>
      </CardHeader>
      <CardContent>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <Label htmlFor="labour">
                {language === 'english' ? 'Labour' : 'கூலி'} *
              </Label>
              <Select value={formData.labour} onValueChange={(value) => setFormData({ ...formData, labour: value })}>
                <SelectTrigger>
                  <SelectValue placeholder={language === 'english' ? 'Select labour' : 'கூலியை தேர்ந்தெடுக்கவும்'} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Select value={formData.supplier_id} onValueChange={(value) => setFormData({ ...formData, supplier_id: value })}>
                <SelectTrigger>
                  <SelectValue placeholder={language === 'english' ? 'Select supplier' : 'சப்ளையரை தேர்ந்தெடுக்கவும்'} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setFormData({ ...formData, full_weight: e.target.value })}
                required
