    ...
    Functions: {
      ...
      get_user_accessible_pages:
        | {
            ...
            Returns: {
              error: true
            } & "Could not choose the best candidate function between: public.get_user_accessible_pages(_user_id => text), public.get_user_accessible_pages(_user_id => uuid). Try renaming the parameters or the function itself in the database so function overloading can be resolved"[]
          }
        | {
            Args: { _user_id: string }
            Returns: {
