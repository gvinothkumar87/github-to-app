...
export const MobileCreditNoteForm = () => {
  ...
  const { data: creditNotes, create: createCreditNote } = useEnhancedOfflineData('offline_credit_notes', [], { autoSync: true });
  ...
  React.useEffect(() => {
    const fetchPreviewNoteNo = async () => {
      const noteNo = await generateCreditNoteNo(formData.mill, new Date(formData.note_date));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const mappedNotes = (creditNotes || []).map((note: any) => ({
